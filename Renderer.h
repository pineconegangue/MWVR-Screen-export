...
// Shared control block for export (must match Main.cpp)
struct ExportControlBlock
{
    uint32_t SelectedWidgetIndex; // Which widget to display (0-9)
    uint32_t WidgetCount;         // How many widgets are active
    ...
    struct WidgetInfo
    {
        ...
        uint32_t Width;
        uint32_t Height;
        uint8_t Type; // WidgetType enum value
        bool Active;
        uint8_t Padding[2]; // Alignment
    } Widgets[10];          // 4 crosshairs + 6 screens
...
{
  private:
    // Shared texture infrastructure for selected widget
    ...
    SharedFrameBuffer SharedBuffers[10]; // Support up to 10 widgets for export

    std::atomic<bool> SharedBufferReady{false};
    std::atomic<bool> ExportEnabled{false};
    ExportControlBlock *ExportControl = nullptr; // Pointer to MMF

    bool EnsureSharedTextureForWidget(ID3D11Device *device, int widgetIndex, uint32_t width, uint32_t height);
    void CopyToSharedTextureForWidget(ID3D11DeviceContext *ctx, int widgetIndex, ID3D11Texture2D *source);

  public:
    Renderer() = default;

    void RenderHUD(HUDWidgetRenderData widgets[10], // Changed from [4] to [10]
                   std::vector<MarkerWidgetRenderData> markerWidgets[3], float brightness, float zoomLevel,
                   EStereoscopicPass pass, uint64_t frameNumber, const FScreenPassTextureViewportParameters &viewport,
                   FRDGTexture *sceneColor, FRDGTexture *renderTarget, bool inMech);

    void EnableExport(bool enable)
    {
        ExportEnabled.store(enable);
    }
    bool IsExportEnabled() const
    {
        return ExportEnabled.load();
    }
    void SetExportControl(ExportControlBlock *control)
    {
  ...
  private:
    ...
    static bool CompileAndCreateVertexShader(ID3D11Device *dvc, const wchar_t *filename, ID3D11VertexShader **vs,
                                             ID3DBlob **b);
    static bool CompileAndCreatePixelShader(ID3D11Device *dvc, const wchar_t *filename, ID3D11PixelShader **ps,
                                            ID3DBlob **b);
    bool EnsurePipeline(ID3D11Device *dvc);
    void RenderMarkersToCurrentRT(ID3D11DeviceContext *ctx, const std::vector<MarkerWidgetRenderData> *markers,
                                  int32_t eye) const;
    static void CreateZoomCameraMarkerRT(ID3D11Device *dvc, int32_t width, int32_t height,
